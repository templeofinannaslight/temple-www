...
def fetch_directus_posts(directus_url: str, token: str, collection: str, verify_ssl: bool) -> list[dict]:
...
def main():
    ...
    parser.add_argument(
        ...
        help='Directus API token'
    )
    parser.add_argument(
        '--collection',
        default='Mikkis_Mess',
        help='Directus collection name'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    posts = fetch_directus_posts(args.directus_url, args.token, args.collection, verify_ssl)
